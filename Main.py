    ...
    for sim_i in simulations:
        ...
        for distr in distr_types:
            for theta in theta_values:
                ...
                # Load data
                for sim in range(1, 4):  # Number of data samples per setting
                    data_path = f"sim_data/sim_{sim_i}/{distr}_theta_{theta}_sim_{sim}.pkl"
                    x_train, x_validation, x_test, x, y_train, y_validation, y_test, y = joblib.load(data_path)
                    ...
                    x_test_scaled = StandardScaler().fit(x_combined).transform(x_test)

                    # Run QR-Lasso
                    result_qr = qr_lasso(x_combined, y_combined, theta=theta, n_sampler=13000, n_burn=3000, thin=10)
                    if theta not in qr_results[sim_i]:
                        qr_results[sim_i][theta] = {}
                    qr_results[sim_i][theta][distr] = {
                        "beta": np.mean(result_qr["beta"], axis=0),
                        "tz": np.mean(result_qr["tz"]),
                        "tau": np.mean(result_qr["tau"]),
                        "eta2": np.mean(result_qr["eta2"], axis=0),
                        "beta_samples": result_qr["beta"]
                    }

                    # Run Lasso
                    lasso_model = LassoCV(cv=5).fit(x_combined, y_combined)
                    if theta not in lasso_results[sim_i]:
                        lasso_results[sim_i][theta] = {}
                    lasso_results[sim_i][theta][distr] = {
                        "beta": lasso_model.coef_,
                        "alpha": lasso_model.alpha_
                    }

    # Save results
    joblib.dump(qr_results, "sim_data/qr_results.pkl")
    joblib.dump(lasso_results, "sim_data/lasso_results.pkl")

    # Step 3: Plot results
    plot_results(qr_results, lasso_results)


def plot_results(qr_results, lasso_results):
    """
    Plot comparison of QR-Lasso and Lasso results for each simulation, highlighting noise distributions and quantiles.
    """
    for sim_i, qr_res in qr_results.items():
        print(f"Visualizing results for Simulation {sim_i}")

        # Prepare subplots for posterior estimates by distribution and theta
        for theta in qr_res:
            for distr in qr_res[theta]:
                qr_beta = qr_res[theta][distr]["beta"]
                lasso_beta = lasso_results[sim_i][theta][distr]["beta"]

                # Create a figure for QR-Lasso vs Lasso comparison
                plt.figure(figsize=(18, 12))
                plt.subplot(2, 3, 1)
                plt.plot(qr_beta, label=f"QR-Lasso ({distr})", alpha=0.7, color="blue")
                plt.plot(lasso_beta, label=f"Lasso ({distr})", alpha=0.7, color="red")
                plt.title(f"Sim {sim_i} | Noise: {distr} | θ={theta}")
                plt.xlabel("Coefficient Index")
                plt.ylabel("Value")
                plt.legend()
                plt.savefig(f"figures/sim_{sim_i}_theta_{theta}_{distr}_noise_effect.png")  # Save the plot as PNG

                # Boxplots for coefficient distributions across cases
                qr_betas = np.array([res["beta"] for res in qr_res[theta].values()])
                lasso_betas = np.array([lasso_results[sim_i][theta][distr]["beta"] for distr in lasso_results[sim_i][theta]])

                plt.figure(figsize=(12, 6))
                plt.boxplot([qr_betas.flatten(), lasso_betas.flatten()], labels=["QR-Lasso", "Lasso"], patch_artist=True)
                plt.title(f"Coefficient Distribution Comparison (QR-Lasso vs. Lasso) - Sim {sim_i} | θ={theta}")
                plt.ylabel("Coefficient Value")
                plt.savefig(f"figures/sim_{sim_i}_theta_{theta}__{distr}_coefficient_distribution.png")  # Save the plot as PNG
                # plt.show()

                # Posterior density plots (by noise type and θ values)
                plt.figure(figsize=(12, 6))
                for coef_idx in range(qr_res[theta][distr]["beta_samples"].shape[1]):
                    sns.kdeplot(qr_res[theta][distr]["beta_samples"][:, coef_idx], label=f"θ={theta} ({distr})", alpha=0.4)
                plt.title(f"Posterior Densities (QR-Lasso) by Noise Type and θ - Sim {sim_i}")
                plt.xlabel("Coefficient Value")
                plt.ylabel("Density")
                plt.legend(fontsize="small")
                plt.savefig(f"figures/sim_{sim_i}_theta_{theta}__{distr}_posterior_density.png")  # Save the plot as PNG
                # plt.show()

                # Trace plot for QR-Lasso convergence diagnostics
                plt.figure(figsize=(12, 6))
                for coef_idx in range(qr_res[theta][distr]["beta_samples"].shape[1]):
                    plt.plot(qr_res[theta][distr]["beta_samples"][:, coef_idx], alpha=0.6, label=f"Beta {coef_idx + 1}")
                plt.title(f"Trace Plot (QR-Lasso Coefficients) - Sim {sim_i}, Noise: {distr}, θ={theta}")
                plt.xlabel("Iteration")
                plt.ylabel("Coefficient Value")
                plt.legend()
                plt.savefig(f"figures/sim_{sim_i}_theta_{theta}__{distr}_trace_plot.png")  # Save the plot as PNG
                # plt.show()
...
def process_real_data_with_qr_lasso():
    """
    Process California housing dataset and run both Lasso and QR-Lasso on it.
    """
    from sklearn.datasets import fetch_california_housing
    from sklearn.model_selection import KFold
    from sklearn.preprocessing import StandardScaler
    ...
    # Iterate through different theta values for QR-Lasso
    for theta in [0.1, 0.3, 0.5]:
        for train_index, test_index in kf.split(X):
            X_train, X_test = X[train_index], X[test_index]
            y_train, y_test = y[train_index], y[test_index]
            ...
            lasso_errors.append(np.mean(np.abs(y_test - y_pred_lasso)))  # Mean Absolute Error
            lasso_betas.append(lasso.coef_)

            # Fit QR-Lasso 
            result_qr = qr_lasso(X_train_scaled, y_train, theta=theta, n_sampler=13000, n_burn=3000, thin=10)
            qr_lasso_betas.append(np.mean(result_qr["beta"], axis=0))  # Mean beta across chains
            y_pred_qr = X_test_scaled @ np.mean(result_qr["beta"], axis=0)
            qr_lasso_errors.append(np.mean(np.abs(y_test - y_pred_qr)))  # Mean Absolute Error

        print(f"Results for Theta {theta}:")
        print(f"  Lasso - Mean Absolute Error: {np.mean(lasso_errors):.4f}")
        print(f"  QR-Lasso - Mean Absolute Error: {np.mean(qr_lasso_errors):.4f}")

    # Save results for later analysis
    joblib.dump(lasso_betas, "real_data/lasso_betas_real.pkl")
    joblib.dump(qr_lasso_betas, "real_data/qr_lasso_betas_real.pkl")

    # Step 3: Plot results
    plot_results(lasso_errors, qr_lasso_errors, lasso_betas, qr_lasso_betas)


def plot_results(lasso_errors, qr_lasso_errors, lasso_betas, qr_lasso_betas):
    """
    Plot comparison of Lasso and QR-Lasso results.
    """
    # Plot boxplots of errors for Lasso and QR-Lasso
    plt.figure(figsize=(12, 6))
    plt.boxplot([lasso_errors, qr_lasso_errors], labels=["Lasso", "QR-Lasso"], patch_artist=True)
    plt.title("Mean Absolute Error Comparison (Lasso vs QR-Lasso)")
    plt.ylabel("Mean Absolute Error")
    plt.savefig("figures_real/mae_comparison.png")
    # plt.show()

    # Boxplot comparison of coefficients for Lasso and QR-Lasso
    lasso_betas = np.array(lasso_betas)
    qr_lasso_betas = np.array(qr_lasso_betas)
    plt.figure(figsize=(12, 6))
    plt.boxplot([lasso_betas.flatten(), qr_lasso_betas.flatten()], labels=["Lasso", "QR-Lasso"], patch_artist=True)
    plt.title("Coefficient Estimates Comparison (Lasso vs QR-Lasso)")
    plt.ylabel("Coefficient Value")
    plt.savefig("figures_real/coefficients_comparison.png")
    # plt.show()

    # Plot the posterior distributions of QR-Lasso coefficients
    plt.figure(figsize=(12, 6))
    for i, beta in enumerate(qr_lasso_betas):
        plt.plot(beta, alpha=0.5, label=f"QR-Lasso Sim {i+1}")
    plt.title("Posterior Estimates (QR-Lasso)")
    plt.xlabel("Coefficient Index")
    plt.ylabel("Value")
    plt.legend(loc="upper right", fontsize="small")
    plt.savefig("figures_real/qr_lasso_posteriors.png")
    # plt.show()

    # Coefficient estimates for Lasso
    plt.figure(figsize=(12, 6))
    for i, beta in enumerate(lasso_betas):
        plt.plot(beta, alpha=0.5, label=f"Lasso Sim {i+1}")
    plt.title("Coefficient Estimates (Lasso)")
    plt.xlabel("Coefficient Index")
    plt.ylabel("Value")
    plt.legend(loc="upper right", fontsize="small")
    plt.savefig("figures_real/lasso_posteriors.png")
    # plt.show()


if __name__ == "__main__":
    process_real_data_with_qr_lasso()
