...
from pymc import Model, sample, find_MAP
...
def generate_data(sim_i):
    np.random.seed(12345)
    if sim_i == 2:  #why is this check ? this needs to be extended to the other sim
        for distr in ["normal", "normalmix", "laplace", "laplacemix"]:
            for theta in [0.1, 0.3, 0.5]:
                for sim in range(1, n_sim + 1):
                    ...
                    save_path = f"Bayesian_stats_project/data/sim_data/sim_{sim_i}/{distr}_theta_{theta}_sim_{sim}.pkl"
...
def perform_analysis(sim_i):
    ...
    for distr in ["normal", "normalmix", "laplace", "laplacemix"]:
        for theta in [0.1, 0.3, 0.5]:
            for sim in range(1, n_sim + 1):
                data_path = f"Bayesian_stats_project/data/sim_data/sim_{sim_i}/{distr}_theta_{theta}_sim_{sim}.pkl"
                ...
                # Save results
                result_path = f"Bayesian_stats_project/Results/sim_data/sim_{sim_i}/{distr}_theta_{theta}_results.pkl"
